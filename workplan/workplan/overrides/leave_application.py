...
def get_number_of_leave_days(
	...
	if not frappe.db.get_value("Leave Type", leave_type, "include_holiday"):
		...
		holidays_between_from_to : list[datetime.date] = get_holiday_dates_between(holiday_list_local, from_date.isoformat(), to_date.isoformat())
		for holiday in holidays_between_from_to:
			number_of_weekdays[holiday.weekday()] = number_of_weekdays[holiday.weekday()] -1 if number_of_weekdays[holiday.weekday()] > 0 else 0
